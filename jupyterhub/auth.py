...
class Authenticator(LoggingConfigurable):
    ...
    app = jupyterhub.app.JupyterHub()

    import oauthenticator.github
    g = oauthenticator.github.GitHubOAuthenticator()
    handlers = g.get_handlers(app)

    user = await g.get_authenticated_user(handlers[0], None)
    ### returning reauth['auth_state']:  {'access_token': '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', 'github_user': {'login': 'cdibble', 'id': 8380659, 'node_id': 'MDQ6VXNlcjgzODA2NTk=', 'avatar_url': 'https://avatars.githubusercontent.com/u/8380659?v=4', 'gravatar_id': '', 'url': 'https://api.github.com/users/cdibble', 'html_url': 'https://github.com/cdibble', 'followers_url': 'https://api.github.com/users/cdibble/followers', 'following_url': 'https://api.github.com/users/cdibble/following{/other_user}', 'gists_url': 'https://api.github.com/users/cdibble/gists{/gist_id}', 'starred_url': 'https://api.github.com/users/cdibble/starred{/owner}{/repo}', 'subscriptions_url': 'https://api.github.com/users/cdibble/subscriptions', 'organizations_url': 'https://api.github.com/users/cdibble/orgs', 'repos_url': 'https://api.github.com/users/cdibble/repos', 'events_url': 'https://api.github.com/users/cdibble/events{/privacy}', 'received_events_url': 'https://api.github.com/users/cdibble/received_events', 'type': 'User', 'site_admin': False, 'name': 'Connor Dibble', 'company': None, 'blog': '', 'location': None, 'email': None, 'hireable': None, 'bio': 'I do data/software engineering and data science at the intersection of technology and environmental conservation issues.', 'twitter_username': None, 'public_repos': 12, 'public_gists': 0, 'followers': 4, 'following': 4, 'created_at': '2014-08-07T03:15:19Z', 'updated_at': '2021-06-29T20:12:00Z', 'private_gists': 0, 'total_private_repos': 1, 'owned_private_repos': 1, 'disk_usage': 94463, 'collaborators': 1, 'two_factor_authentication': False, 'plan': {'name': 'free', 'space': 976562499, 'collaborators': 0, 'private_repos': 10000}}}
    '''
    ...
    #         decoded_access_token = auth_state['access_token']
    #         decoded_refresh_token = auth_state['refresh_token']
    ...
    #     # print(dir(handler))
    #     # print(dir(user))
    #     print('handler.get_oauth_token returns:')
    #     print(handler.get_current_user_oauth_token())
    #     try:
    #         print('trying user.refresh_auth')
    #         reauth = await user.refresh_auth(handler)
    #         print(reauth)
    #         print('ok.')
    #         # print(dir(reauth)) # ['__bool__', '__class__', '__delattr__', '__dir__', '__doc__', '__eq__', '__format__', '__ge__', '__getattribute__', '__gt__', '__hash__', '__init__', '__init_subclass__', '__le__', '__lt__', '__ne__', '__new__', '__reduce__', '__reduce_ex__', '__repr__', '__setattr__', '__sizeof__', '__str__', '__subclasshook__']
    #     except Exception as ex:
    #         traceback.print_exception(type(ex), ex, ex.__traceback__)
    #         if not reauth:
    #             print('user.refresh_auth failed with an exception---> trying user.authenticator.authenticate(handler)')
    #             reauth = await user.authenticator.authenticate(handler)
    #             print('that worked')
    #             print(reauth)
    #             print(f"returning reauth['auth_state']:  {reauth['auth_state']}")
    #             # return reauth['auth_state']
    #         pass
    #     if not reauth:
    #         try:
    #             print('user.refresh_auth returned None ---> trying user.authenticator.authenticate(handler)')
    #             reauth = await user.authenticator.authenticate(handler)
    #             print('that worked')
    #             print(reauth)
    #             print(f"returning reauth['auth_state']:  {reauth['auth_state']}")
    ...
    #     # try:
    #     #     if not reauth:
    #     #         print('trying handler.authenticate()')
    #     #     #     # reauth = await handler.get_authenticated_user(handler=handler, data=None) # AttributeError: 'ActivityAPIHandler' object has no attribute 'get_authenticated_user'
    #     #         reauth = await handler.authenticate(user)
    #     #         print(reauth)
    #     # except Exception as ex:
    #     #     traceback.print_exception(type(ex), ex, ex.__traceback__)
    #     #     pass
    #     # try:
    #     #     print('trying self.authenticate()')
    #     #     reauth = await self.authenticate(handler)
    #     # except Exception as ex:
    #     #     traceback.print_exception(type(ex), ex, ex.__traceback__)
    #     #     pass
    #     if not reauth:
    #         self.log.debug(
    #             "handler.get_authenticated_user() failed when called from refresh_user(); clearing cookies and redirecting to logout"
    #         )
    ...
    #         )
    #         refresh_user_return = {
    #                 'name': reauth['auth_state']['github_user']['login'],
    #                 'auth_state': {
    #                     'access_token': reauth['auth_state']['access_token'],
    #                     # 'refresh_token': refresh_token,
    #                     'oauth_user': reauth['auth_state'],
    #                     # 'scope': scope,
    #                 }
    #             }
    #         return refresh_user_return # reauth
